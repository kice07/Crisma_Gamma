...
if ($_POST["action"] == "getsub") {
    ...
    echo $output;
} elseif ($_POST["action"] == "whishlist") {
    $id = $_POST['id'];
    $free_id = $_SESSION['unique_freelancer_id'];

    $select_query = mysqli_query($conn, "SELECT * FROM whishlist WHERE job_id='$id' AND free_id='$free_id'");
    if (mysqli_num_rows($select_query)>0) {
        $delete_query =  mysqli_query($conn, "DELETE FROM whishlist WHERE job_id='$id' AND free_id='$free_id'");
        if ($delete_query) {
            echo "success delete";
        } else {
        ...
        $insert_query = mysqli_query($conn, "INSERT INTO whishlist(job_id, free_id) VALUES ('$id', '$free_id')");
        if ($insert_query) {
            echo "success insert";
        } else {
            echo mysqli_error($conn);
