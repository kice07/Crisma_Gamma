...
inputImage.addEventListener("change", function (e) {
    ...
    if (input.files && input.files[0]) {
        ...
        xhr.open("POST", "admin_dashboard_assets/php_checking/edit-profile.php", true);
...
function updateInfo(trigger) {
    ...
    // console.log(inputBox.querySelector(".name").value);
    ...
    if (emptyCounter == 4) {
        if (regxName.test(inputBox.querySelector(".name").value)) {
            if (regexMail.test(inputBox.querySelector(".mail").value)) {
                if ((inputBox.querySelector(".language").value).toLowerCase().substr(0, 2) != "fr" ||
                    ...
                    sendInfo.append('name', inputBox.querySelector(".name").value);
                    ...
                    var message = "choisissez une langue netre l'anglais et le français";
