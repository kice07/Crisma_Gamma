...
document.addEventListener("DOMContentLoaded", () => {
    ...
    setInterval(() => {
        ...
        xhr.open("GET", "admin_dashboard_assets/php_checking/user.php", true);
        xhr.onload = () => {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    ...
                    searchBarUser.onkeyup = () => {
                        ...
                        xhr.open("POST", "admin_dashboard_assets/php_checking/search.php", true);
                        xhr.onload = () => {
                            if (xhr.readyState === XMLHttpRequest.DONE) {
                                if (xhr.status === 200) {
                                    let data = xhr.response;
...
function loadMsg(chatBox, persoId) {
    ...
    let xhr = new XMLHttpRequest();
    xhr.open("POST", "admin_dashboard_assets/php_checking/get-chat.php", true);
    xhr.onload = () => {
        if (xhr.readyState === XMLHttpRequest.DONE) {
            if (xhr.status === 200) {
                let data = xhr.response;
...
function sendMsg(file, Ftype, Fsize, Fname, incoming_id, inputField, chatBox) {
    ...
    sendMessage.append('freelancer', incoming_id);
    ...
    xhr.open("POST", "admin_dashboard_assets/php_checking/insert-chat.php", true);
    xhr.onload = () => {
        if (xhr.readyState === XMLHttpRequest.DONE) {
            if (xhr.status === 200) {
                console.log("done");
...
function createChat(element) {
    ...
    // unread matter 
    let xhr = new XMLHttpRequest();
    xhr.open("POST", "admin_dashboard_assets/php_checking/update.php", true);
    xhr.onload = () => {
        if (xhr.readyState === XMLHttpRequest.DONE) {
            if (xhr.status === 200) {
                let data = xhr.response;
